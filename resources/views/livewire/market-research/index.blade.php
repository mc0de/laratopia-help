    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="MarketResearch" format="csv" />
                <livewire:excel-export model="MarketResearch" format="xlsx" />
                <livewire:excel-export model="MarketResearch" format="pdf" />
            @endif


            @can('market_research_create')
                <x-csv-import route="{{ route('admin.market-researchs.csv.store') }}" />
            @endcan
    ...
    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($marketResearchs as $marketResearch)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $marketResearch->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $marketResearch->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('market_research_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.market-researchs.show', $marketResearch) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('market_research_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.market-researchs.edit', $marketResearch) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('market_research_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $marketResearch->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
